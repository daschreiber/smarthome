import { getStates, type HaState } from "./ha";
import { reconcileEntityIds, registry } from "./registry";
import type { Role } from "./users";
import { canOperateLocks } from "./permissions";
import { coolmasterEntityId } from "./coolmaster";
import { changeoverStatus, modeFromRelayState, relayEntityId } from "./changeover";
import { saunaConfigured, saunaScheduleStatus, saunaStatus } from "./sauna";
import { noiseConfigured, noiseStatus } from "./whitenoise";
import { bedConfigured, bedSideForDeviceId } from "./eightsleep";
import { zoneRoomFor } from "./audio";
import { unverifiedFor } from "./knxLights";
import { deviceUnreachable, entityUnreachable } from "./reachability";

/**
 * Full visible-device snapshot joined with live HA state — the one truth
 * the UI polls (GET /api/home) and the MCP server reads (lib/mcp). One bulk
 * /api/states call, not N per-entity reads. Throws when Home Assistant is
 * unreachable; the callers decide how to say so.
 */
...
export async function homeSnapshot(role: Role) {
  ...
  const devices = registry()
    ...
    .map((d) => {
      ...
      return {
        ...
        ...(d.kind === "lock" ? { lockAllowed: canOperateLocks(role) } : {}),
  ...
  return {
    devices, role, floorHeatingRooms, floorModes,
    coverStateTrusted: process.env.COVER_STATE_TRUSTED === "1",
  };
}

export type HomeSnapshot = Awaited<ReturnType<typeof homeSnapshot>>;
export type HomeDevice = HomeSnapshot["devices"][number];
