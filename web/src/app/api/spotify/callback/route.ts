...
import { appBaseUrl, consumeState, exchangeCode, spotifyRedirectUri } from "@/lib/spotify";
...
export async function GET(req: NextRequest) {
  ...
  if (denied) return NextResponse.redirect(`${appBaseUrl()}/more?spotify=denied`);
  ...
  try {
    ...
    return NextResponse.redirect(`${appBaseUrl()}/more?spotify=linked`);
  } catch (err) {
