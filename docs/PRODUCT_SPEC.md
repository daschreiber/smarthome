...
- ~~Yale lock model and connection type~~ Resolved from photos: Yale Linus retrofit lock + Smart Keypad + Smart Video Doorbell, all in the Yale Home ecosystem (not Control4 Zigbee). The Yale Home integration in Home Assistant covers them, with the doorbell acting as the bridge. Remaining detail: confirm in the Yale Home app whether the lock is a Linus L1 or L2 (L2 adds a local Matter option).
