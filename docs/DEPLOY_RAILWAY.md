...
4. **Variables** tab — add:
   ...
   | `HA_HOOK_KEY` | optional — `openssl rand -hex 32`; the key Home Assistant presents (`x-hook-key`) when it relays a wall-keypad Night/Morning press to `POST /api/artframes`, so the picture Frames follow the wall as they follow the app (`ha/artframes_keypad.yaml`, `ha/README.md`). The same string goes into the Green's `secrets.yaml`. Deliberately not `APP_KEY`: this one buys a Frame sweep and nothing else |
